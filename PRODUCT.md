...
- OSF Registries via the share.osf.io index-card search (CORS-open, JSON-LD; resourceType=Registration, bepress Psychology subject for psych scope, `-dateCreated` sort, cursor links for paging). The index has no OR and no year or language filter, so each variable sends its first ticked variant and year/relevance are checked client-side. AsPredicted has no search API and is not covered.
- Study designs offered: meta-analysis, systematic review, primary quantitative (rct / any trial / observational), qualitative, preregistration (registered plans and protocols, not empirical papers that were preregistered). English-only toggle maps to `[la]`, `LANG:eng`, `language:en`, `LA=(English)`.
